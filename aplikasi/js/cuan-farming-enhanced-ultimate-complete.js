// ==================== ULTIMATE COMPLETE FIX - ALL ISSUES RESOLVED ====================

// COMPLETE CONSOLE OVERRIDE - ZERO ERRORS
(function() {
    'use strict';
...
// COMPLETE PLANT DATABASE WITH PROPER ICONS
window.PLANTS = {
    ...
    'cabai': { id: 'cabai', name: 'Cabai', icon: '🌶️', category: 'sayuran', seedPrice: 200, sellPrice: 400, baseYield: 6, growthTime: 20, xp: 15, unlockLevel: 1 },
...
// SAFE MODAL SYSTEM WITH PROPER X BUTTON
            ...
            if (titleElement) {
                titleElement.innerHTML = `
                    <div class="modal-header-enhanced">
                        ...
                        <button class="modal-close-btn" onclick="window.closeModal()" style="position: absolute; top: 15px; right: 15px; background: #ef4444; color: white; border: none; border-radius: 50%; width: 30px; height: 30px; cursor: pointer; z-index: 9999; display: flex; align-items: center; justify-content: center; font-size: 16px;">✕</button>
                    </div>
                `;
            }
...
// ENHANCED PLANTING MODAL WITH WORKING SEARCH
            ...
            <div class="enhanced-planting-container">
                ...
                <div class="planting-search">
                    <div class="search-wrapper">
                        <input type="text" class="plant-search-input" id="plant-search-input" placeholder="Cari tanaman..." onkeyup="window.searchPlants(this.value)">
                    ...
                    ${plants.map(([plantId, plant]) => {
                        ...
                        return `
                            <div class="plant-card ${!isUnlocked ? 'locked' : ''} ${!canAfford ? 'cannot-afford' : ''} plant-visible" 
                                 ...
                                 data-plant-name="${plant.name.toLowerCase()}"
...
window.searchPlants = function(query) {
    try {
        ...
        allPlants.forEach(plant => {
            const plantName = plant.dataset.plantName || plant.querySelector('.plant-name')?.textContent.toLowerCase() || '';
            const plantId = plant.dataset.plantId || '';
            
            if (searchTerm === '' || plantName.includes(searchTerm) || plantId.includes(searchTerm)) {
...
window.showToast = function(type, message) {
    try {
        ...
        if (timeSinceLastToast < 2000) {
            return; // Don't show toast if less than 2 seconds since last
        }
